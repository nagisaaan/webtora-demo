...
def calculate_yaw_from_points(left_pt, right_pt):
    ...
    # MediaPipeの座標系とVRChatの座標系に合わせて調整
    # 通常、右方向が0度または90度。ここではdxが正なら正面(0度)に近いと仮定
    angle = math.degrees(math.atan2(dz, dx))
    
    # 補正: VRChatではトラッカーが正面を向いているとき、Y回転は0や180になる
    # ここでは経験則的に調整。ねじれが逆なら符号を反転させる
    return -angle
...
def update_virtual_pose():
    ...
    # 足などの回転は簡易的に腰に合わせるか、0のままにする（今回は0のまま）
    # 必要ならここに追加可能
...
def apply_smoothing(key, target_pos, target_rot):
    ...
    # 回転の補間 (単純線形補間)
    # 本来はクォータニオン推奨ですが、簡易的にEuler角で補間します
...
def send_pose_to_vrchat():
    ...
    for key, value in pose_virtual_transforms.items():
        ...
        # 全体補正 (X, Y, Z)
        final_pos[0] += MANUAL_X_OFFSET      # 左右 (追加)
...
class AppGUI:
    def __init__(self, root):
        ...
        self.scale_x = tk.Scale(global_adj_frame, from_=-2.0, to=2.0, resolution=0.01, 
                                orient=tk.HORIZONTAL, label="左右 (X) オフセット [NEW]", command=self.update_offsets)
